...
interface DeleteModalProps {
  isOpen: boolean;
  tramitationInfo: string;
  onClose: () => void;
  onConfirm: () => void;
}

export function DeleteModal({ isOpen, tramitationInfo, onClose, onConfirm }: DeleteModalProps) {
      ...
      >
        <div className="p-6">
          ...
          <div className="space-y-4">
            {/* Tramitação a ser excluída */}
            <div>
              <p className="text-sm text-gray-700 mb-2">
                Você está prestes a excluir a seguinte tramitação:
              </p>
              <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                <p className="text-sm font-semibold text-gray-900">{tramitationInfo}</p>
              </div>
            </div>
              ...
              <div className="flex gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-yellow-800 mt-1">
                    Esta ação não poderá ser desfeita e a tramitação será removida permanentemente.
                  </p>
                </div>
              </div>
            </div>
