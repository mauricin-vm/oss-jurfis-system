...
export function SessionForm({ initialData }: SessionFormProps) {
  ...
  const onSubmit = async (data: SessionFormValues) => {
    try {
      ...
      const successMessage = initialData?.id ? 'Sessão atualizada com sucesso' : 'Sessão criada com sucesso';
      toast.success(successMessage);

      if (initialData?.id) {
        router.push(`/ccr/sessoes/${initialData.id}`);
      } else {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end gap-4 pt-4">
          <Button
            type="button"
            variant="outline"
            onClick={() => router.push(initialData?.id ? `/ccr/sessoes/${initialData.id}` : '/ccr/sessoes')}
            disabled={loading}
