...
export function DecisionTable({ data, loading, onRefresh, onNewDecision, userRole }: DecisionTableProps) {
  ...
  const [decisionToDelete, setDecisionToDelete] = useState<{ id: string; identifier: string } | null>(null);
  ...
  const handleDeleteClick = (id: string, identifier: string) => {
    setDecisionToDelete({ id, identifier });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      setDeletingId(decisionToDelete.id);
      const response = await fetch(`/api/ccr/vote-decisions/${decisionToDelete.id}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok) {
        ...
        toast.error(data.message || 'Erro ao remover voto');
      }
    } catch (error) {
  ...
  const filteredData = data.filter((decision) => {
    const typeMatch = typeFilter === 'all' ||
      (typeFilter === 'PRELIMINAR' && decision.type === 'PRELIMINAR') ||
      (typeFilter === 'MERITO' && decision.type === 'MERITO') ||
      (typeFilter === 'OFICIO' && decision.type === 'OFICIO');

    const statusMatch = statusFilter === 'all' ||
      (statusFilter === 'active' && decision.isActive) ||
      (statusFilter === 'inactive' && !decision.isActive);

    const searchMatch = !searchQuery ||
      decision.identifier.toLowerCase().includes(searchQuery.toLowerCase());

    return typeMatch && statusMatch && searchMatch;
  });

  // Ordenar dados por tipo e depois por identificador
  const sortedData = filteredData.sort((a, b) => {
    if (a.type !== b.type) {
      return a.type === 'PRELIMINAR' ? -1 : 1;
    }
  ...
  // Contadores
  const preliminaryCount = data.filter((d) => d.type === 'PRELIMINAR').length;
  const meritCount = data.filter((d) => d.type === 'MERITO').length;
  const oficioCount = data.filter((d) => d.type === 'OFICIO').length;
  ...
  return (
    <div className="space-y-4">
      {/* Botões de Busca, Filtros e Novo Voto */}
      <div className="flex justify-end gap-2">
        ...
        <div className="relative flex items-center">
          ...
          >
            <div className="relative w-full h-8">
              ...
              />
              <TooltipWrapper content="Buscar por identificador">
                <Button
                  variant="outline"
                  size="sm"
          ...
          <DropdownMenuContent align="end" className="w-[280px] sm:w-[320px] p-0">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent className="rounded-md">
                    ...
                    <SelectItem value="PRELIMINAR" className="cursor-pointer h-9">
                      Preliminar ({preliminaryCount})
                    </SelectItem>
                    <SelectItem value="MERITO" className="cursor-pointer h-9">
                      Mérito ({meritCount})
                    </SelectItem>
            ...
            <div className="border-t p-3">
              <div className="text-xs text-muted-foreground text-center">
                {filteredData.length} {filteredData.length === 1 ? 'voto' : 'votos'} encontrado{filteredData.length !== 1 ? 's' : ''}
              </div>
            </div>
          </DropdownMenuContent>
        </DropdownMenu>
        ...
        <Button size="sm" onClick={onNewDecision} className="h-8 gap-2 cursor-pointer">
          ...
          <span className="hidden sm:inline">Novo Voto</span>
        </Button>
      </div>
      ...
      {filteredData.length === 0 ? (
        <div className="rounded-lg border bg-card text-card-foreground shadow-sm">
          <div className="p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">Clique em "Novo Voto" para adicionar.</p>
          </div>
        </div>
      ) : (
        <>
                ...
                <TableBody>
                  {paginatedData.map((decision) => (
                    <TableRow key={decision.id} className="bg-white hover:bg-muted/40 min-h-[49px]">
                      <TableCell>
                        {decision.type === 'PRELIMINAR' ? 'Preliminar' : decision.type === 'MERITO' ? 'Mérito' : 'Ofício'}
                      </TableCell>
                      <TableCell className="font-medium">
                        {decision.identifier}
                      </TableCell>
                      <TableCell>
                        <Badge variant={decision.isActive ? 'default' : 'secondary'} className="pointer-events-none">
                          {decision.isActive ? 'Ativo' : 'Inativo'}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem
                              onClick={() => router.push(`/ccr/votos/${decision.id}`)}
                              className="cursor-pointer h-9"
                            >
                              <Pencil className="mr-2 h-4 w-4" />
                              Editar
                            </DropdownMenuItem>
                            {canDelete && !decision.isInUse && (
                              <DropdownMenuItem
                                onClick={() => handleDeleteClick(decision.id, decision.identifier)}
      ...
      <DeleteModal
        ...
        decisionName={decisionToDelete?.identifier || ''}
        onClose={() => {
          setIsDeleteModalOpen(false);
          setDecisionToDelete(null);
        }}
